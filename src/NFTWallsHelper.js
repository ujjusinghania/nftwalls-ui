...
export const getSupportedProjects = (callback) => {
    let request = axios.get('http://34.227.161.251/getSupportedProjects').then(res => res).catch(error => error);
...
export const getProjectInformation = (assetProjectName, callback) => {
    ...
    let request = axios.get(`http://34.227.161.251/getProjectInformation?${queryParams}`).then(res => res).catch(error => error);
    request.then((api_response) => {
        callback(api_response.status, api_response.data);
    });
};
